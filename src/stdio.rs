//! Implementation of tokio_io read/write for stdin/stdout streams
...
impl Read for Stdio {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        match self.stdin_buffer_receiver.read(buf) {
            ...
            Err(_) => Err(new_pipe_broken_error()),
        }
    }
}
    ...
    fn start_stdin_thread(stdin_buffer_size: usize) -> rb::Receiver<u8> {
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                match result {
                    Ok(ret_sender) => sender = ret_sender,
                    Err(_) => {
                        trace!("Stdin ring buffer receiver has been dropped, terminating stdin \
                                read thread.");
                        return;
                    }
                }
        ...
        thread::spawn(move || {
            ...
            loop {
                let bytes_read = match receiver.read_some(&mut buffer).wait() {
                    ...
                    Err(_) => {
                        trace!("Stdout ring buffer sender has been dropped, terminating stdout \
                                write thread.");
                        return;
                    }
                };
                ...
                match stdout.write_all(&buffer[..bytes_read]) {
                    Ok(_) => {}
                    Err(e) => {
                        trace!("Error in stdout write thread while writing to stdout, assuming \
                                closed: {:?}",
                               e);
                        return;
                    }
                ...
                match stdout.flush() {
                    Ok(_) => {}
                    Err(e) => {
                        trace!("Error in stdout write thread while flushing stdout, assuming \
                                closed: {:?}",
                               e);
                        return;
                    }
